...
restart:
	...
	microSPIW(0x40 + 59);    // CMD59 (Turn CRC Off)
	microSPIW(0x00);    //
	microSPIW(0x00);
	microSPIW(0x00);    // HVS
	microSPIW(0x00);    // test pattern
	microSPIW(0x91);    // right checksum for CMD8
	dumpSD( 6 );
...
retryCMD41:
	...
	microSPID();
	{
		...
		if( (r & 0x04)  || (!(r&1))  ) 
		{
			//Read in error or state is not yet idle.
			dumpSD(4);
			SDCSDE;
			if( tries++ < 10 ) 
				goto retryCMD41;
	...
	dumpSD(5); //We don't care about the response.
/*
	if( WaitFor( 0 ) )
	{
		//Up to three tries to get sync'd.
